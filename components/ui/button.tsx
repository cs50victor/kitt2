  ...
  'inline-flex items-center justify-center rounded-2xl text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50',
  {
    variants: {
      variant: {
        ...
        'link with underline': 'text-brand underline-offset-4 hover:underline',
      },
      size: {
...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    return (
      // @ts-ignore
      <Comp className={tw(buttonVariants({ variant, size, className }))} ref={ref} {...props} />
    );
  },
);
...
const ButtonLink = ({ variant = 'link', size, href, className, ...props }: ButtonLinkProps) => {
  return (
    <Link className={tw(buttonVariants({ variant, size, className }))} href={href} {...props} />
  );
};
